...
**Status:** Implemented (v0.2 shipped)  
**Version:** 0.2  
**Baseline:** v0.1 (linear clarifier → rewriter → retrieval → answerer → validator pipeline, synchronous REST, no streaming)

> v0.2 is live behind `agent_graph.enabled` (`DHARMIQ_AGENT_GRAPH_V2=true` locally; on by default in `config.beta.yaml`). The v0.1 linear pipeline remains available when the flag is off. See [README](../../README.md) and [deployment.md](../deployment.md) for setup.
